#### Question 1: About the Latency calculations

Now, After the wordaccess is computed by the cost model, which is utilised for the latency computations also as the word is accessed from the memory and MAC computation is performed the required latency is also calculated analytically by the cost model.

I am interested in knowing how this latency calculation is performed analytically by the zigzag cost model by utilising the wordaccess calcualtions and architecture model as shown below
...
memories:
  ...
  sram:
    size: 1048576  # 1 MB, e.g., 8 Mb Data Memory
    r_bw: 128
    w_bw: 128
...
I want a general algorithm that is used for the latency calculations which can be used in the script development and also in the presentation
